...
-- 4.  Contare quante lingue sono parlate in Italia - VERSIONE BREVE
SELECT COUNT(*) as num_lingue
FROM country_languages
WHERE country_id = 107;

-- 4.  Contare quante lingue sono parlate in Italia - VERSIONE UFFICIO COMPLICAZIONE AFFARI SEMPLICI
...
SELECT c.name as nazione, c.area, l.`language` 
FROM `countries` c
...
SELECT c.name as continent, COUNT(c2.country_id) as countries_with_area_over_10k
FROM `continents` c
INNER JOIN `regions` r
...
WHERE c2.area > 10000
GROUP BY continent
ORDER BY countries_with_area_over_10k DESC;